...
이 문서는 브랜치 명명 규칙과 PR 작성 요령, PR 제목 규칙을 정리하는 문서입니다. 커밋 메시지 규칙은 [`commit.md`](./commit.md)를 참고하세요.
...
접두사는 [`commit.md`](./commit.md)의 커밋 접두사 규칙을 그대로 따릅니다.

## Pull Request

### 제목

`[접두사] 작업 내용` 형태로 작성합니다. 접두사는 [`commit.md`](./commit.md)의 커밋 접두사와 동일하되 첫 글자만 대문자로 씁니다.
