...
- 데스크탑 기준 지원 범위는 1920px ~ 1280px이고, 1024px까지는 레이아웃이 깨지지 않는 선에서 허용합니다. 보통 해당 사이트 컨텐츠 영역(Header, 본문 wrapper 등)의 너비 기준으로 봅니다.
- `body`에 `min-width: 102.4rem`(1024px)을 걸어 그 이하로는 줄어들지 않게 하되, `lg`(1024px) 이상에서만 적용합니다(`src/styles/layout.css`). 브레이크포인트 경계값과 강제 최소폭을 동일하게 맞춰야 정확히 그 경계에서 1px 오차로 인한 가로 스크롤이 생기지 않습니다. `lg` 미만은 아래 브레이크포인트 정책에 따라 모바일 레이아웃으로 대응합니다.
