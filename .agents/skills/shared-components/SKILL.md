...
버튼, 인풋, 헤더, 푸터처럼 자체 시각 정체성을 가진 UI 요소면 `components/ui`에 둡니다. 텍스트/이미지 같은 여러 컨텐츠 슬롯의 배치만 정의하는 컴포넌트는 `components/layout`에 둡니다 — 피그마 컴포넌트 이름에 `Layout`이 붙어 있으면 이 경우로 봅니다.
